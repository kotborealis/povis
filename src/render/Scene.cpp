...
void Scene::draw(){
    for(auto scene_nodes_it=nodes.begin();scene_nodes_it!=nodes.end();scene_nodes_it++){
        ...
        GLuint diffuse_size=node.material.texture_diffuse.size();
        GLuint specular_size=node.material.texture_specular.size();

        for(;texture_index<diffuse_size;texture_index++){
            glActiveTexture(GL_TEXTURE0+texture_index);
            std::string name=SSTR("texture_diffuse"<<diffuse_index++);
            glUniform1f(node.material.shader->uniform(name), texture_index);
            node.material.texture_diffuse[texture_index]->use();
        }
